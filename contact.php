...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nom) || !filter_var($email, FILTER_VALIDATE_EMAIL) || empty($message)) {
        echo "<script>alert('Veuillez remplir correctement tous les champs.'); window.location.href = 'https://enzodigiovanni.fr/';</script>";
    } else {
        ...
        if (mail($to, $subject, $content, $headers)) {
            ...
            echo "<script>alert('Erreur lors de l\'envoi du message.'); window.location.href = 'https://enzodigiovanni.fr/';</script>";
        }
    }
}
